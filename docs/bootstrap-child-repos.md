...
The four child repos were created **locally** inside `agents/` with their
...
```bash
for r in claude gpt cursor aider; do
...
## Create and push all four (requires `gh auth login`)
...
1. Create four **empty** repos (no README, no license) at
   https://github.com/new named `dealersource-claude`, `dealersource-gpt`,
   `dealersource-cursor`, `dealersource-aider`.
